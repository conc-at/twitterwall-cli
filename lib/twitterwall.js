...
function sendRequest(url, data, cb){
  request({
    ...
    else if(!error && response.statusCode === 401) console.log('user unauthorized!\nuse the "reconfigure" command to change your login credentials.')
    else cb(error)
  })
}
...
function runProgram(){
    ...
    .action(function(user, message){
      if(user.length > 0 && message.length > 0){
        sendRequest('tweet', {user: user, text: message}, function(err, data){
          if(!err && data && data.success) console.log('faketweet sent.')
  ...
  program
    .command('flash [message] [duration]')
    .description('send a flash message')
    .action(function(message, duration){
      duration = parseInt(duration, 10)
      if(isNaN(duration)) duration = 0
      duration *= 1000
      sendRequest('flash', {message: message, duration: duration}, function(err, data){
        if(!err && data && data.success) console.log('flash message sent.')
